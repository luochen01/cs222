...
RC TEST_RM_13b(const string &tableName)
{
	...
	unsigned char *nullsIndicatorWithNull = (unsigned char *) malloc(
	...
	for (int i = 0; i < numTuples; i++)
	{
		...
		if (i % 10 == 0)
		{
			...
			tupleName += suffix;
			prepareTuple(attrs.size(), nullsIndicatorWithNull, tupleName.length(), tupleName, 0,
					height, 456, tuple, &tupleSize);
		}
		else
		{
			tupleName = "Tester";
			tupleName += suffix;
			prepareTuple(attrs.size(), nullsIndicator, tupleName.length(), tupleName, age, height,
					123, tuple, &tupleSize);
		}
