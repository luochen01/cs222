...
RC RM_TEST_EXTRA_2(const string &tableName, const int nameLength, const string &name, const int age,
		const int height, const int salary, const int ssn)
{
	...
	// Test Insert Tuple
	prepareTupleAfterAdd(attrs.size(), nullsIndicator, nameLength, name, age, height, salary, ssn,
			tuple, &tupleSize);
	...
	{
		cout << "***** Extra Credit Test Case 2 Finished. The result will be examined. *****"
				<< endl << endl;
		free(tuple);
		free(returnedData);
		return success;
